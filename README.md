cryptonote-hongbao-pool
====================

**For the latest features and improvements, please use source code from @SadBatman repo  https://github.com/SadBatman/cryptonote-hongbao-pool/**

High performance Node.js (with native C addons) mining pool for CryptoNote based coins such as Bytecoin, DuckNote, Monero, QuazarCoin, Boolberry, Dashcoin, Hongbao etc..
...
```bash
git clone https://github.com/sumoprojects/cryptonote-hongbao-pool.git pool
cd pool
npm update
```
...
"blockUnlocker": {
    ...
    "coreDevDonation": 0.1 //0.1% donation to send to core devs - works with Bytecoin, Monero, Dashcoin, QuarazCoin, Fantoncoin, AEON, Hongbao, OneEvilCoin
},
...
```bash
node init.js -config=config_hongbao.json
```
...
var irc = "irc.freenode.net/#hongbao";
